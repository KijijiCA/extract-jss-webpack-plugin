...
module.exports.pitch = function(request) {
	...
	if(this[__dirname] === undefined) {
		...
		throw new Error(
			'"extract-json-webpack-plugin" loader is used without the corresponding plugin, ' +
			'refer to https://github.com/webpack/extract-json-webpack-plugin for the usage example'
		);
		...
		if(query.extract !== false) {
			var childFilename = "extract-json-webpack-plugin-output-filename"; // eslint-disable-line no-path-concat
			var publicPath = typeof query.publicPath === "string" ? query.publicPath : this._compilation.outputOptions.publicPath;
			var outputOptions = {
				filename: childFilename,
				publicPath: publicPath
			};
			var childCompiler = this._compilation.createChildCompiler("extract-json-webpack-plugin", outputOptions);
			...
			childCompiler.runAsChild(function(err, entries, compilation) {
				...
				try {
					...
					resultSource = "\nmodule.exports = " + JSON.stringify(resultSource) + ";";
				} catch(e) {
					return callback(e);
				}
